...
const QuanLyKhuyenMai = () => {
  ...
  const [promotions, setPromotions] = useState([
    { id: 'KM001', name: 'Giảm TV 4K', productId: 'SP001', start: '2026-04-20', end: '2026-05-15' },
    { id: 'KM002', name: 'Ưu đãi laptop mùa tựu trường', productId: 'SP005', start: '2026-04-25', end: '2026-05-31' },
  ]);
  ...
  return (
    <div className="page">
      <header className="header">
        ...
        <button className="btn" onClick={() => setForm({ id: `KM${String(promotions.length + 1).padStart(3, '0')}`, name: '', productId: products[0]?.id || '', start: new Date().toISOString().slice(0, 10), end: new Date().toISOString().slice(0, 10) })}>Thêm khuyến mãi</button>
      </header>
      ...
      <section className="card">
        ...
        <div className="table-wrap" style={{ marginTop: 14 }}>
          <table>
            <thead><tr><th>Mã KM</th><th>Tên chương trình</th><th>Sản phẩm</th><th>Bắt đầu</th><th>Kết thúc</th><th>Thao tác</th></tr></thead>
            <tbody>
              {promotions.map((item) => (
                <tr key={item.id}>
                  <td>{item.id}</td>

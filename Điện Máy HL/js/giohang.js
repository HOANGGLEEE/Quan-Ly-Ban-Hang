...
<td style="display:flex; gap:10px; justify-content:center;">
    <div class="thanhtoan" onclick="thanhToanTienMat()">
        Thanh toán tiền mặt
    </div>

    <div class="thanhtoanck" onclick="thanhToanChuyenKhoan()">
...
function thanhToanChuyenKhoan() {
    ...
    if (!c_user || !c_user.products.length) {
        addAlertBox('Không có sản phẩm để thanh toán!', '#ffb400', '#fff', 2000);
        return;
    }
    ...
    // 👉 Lưu tạm tổng tiền (nếu sau này cần hiển thị)
    localStorage.setItem('DonHangDangThanhToan', JSON.stringify({
...
function thanhToanTienMat() {
    thanhToan(); // dùng lại logic cũ y như bạn
}

...
function phanTich_URL_chiTietSanPham() {
    nameProduct = decodeURIComponent(window.location.href.split('?')[1] || ''); // lấy tên + decode
    ...
    // Đổi title
    document.title = nameProduct + ' - Thế giới điện máy';
    ...
    // Cập nhật thông số (điện máy) - render linh hoạt theo key/value
    var info = document.getElementsByClassName('info')[0];
    var s = '';

    // Map key -> label tiếng Việt cho dễ đọc
    var labelMap = {
        loai: 'Loại sản phẩm',
        model: 'Model',
        kich_co: 'Kích cỡ',
        ...
        dungtich: 'Dung tích',
        dung_tich: 'Dung tích',
        khoi_luong_giat: 'Khối lượng giặt',
        kieu_may: 'Kiểu máy',
        cong_nghe: 'Công nghệ',
        dien_tieu_thu: 'Điện tiêu thụ',
        ...
        bo_loc: 'Bộ lọc',
        do_on: 'Độ ồn',
        chat_lieu_long: 'Chất liệu lòng',
        so_vung_nau: 'Số vùng nấu',
        nhiet_do: 'Nhiệt độ',
        chuc_nang: 'Chức năng'
    };

    // Nếu dữ liệu detail là object => duyệt hết để hiển thị
    if (sanPhamHienTai.detail && typeof sanPhamHienTai.detail === 'object') {
        for (var key in sanPhamHienTai.detail) {
            if (!sanPhamHienTai.detail.hasOwnProperty(key)) continue;
            var value = sanPhamHienTai.detail[key];
            if (value === undefined || value === null || value === '') continue;

            var label = labelMap[key] || (key.replace(/_/g, ' '));
            s += addThongSo(label, value);
        }
    }
...
function getDetailPromo(sp) {
    switch (sp.promo.name) {
        ...
        case 'moiramat':
            return `Sản phẩm mới ra mắt – mời bạn đến cửa hàng trải nghiệm trực tiếp.`;

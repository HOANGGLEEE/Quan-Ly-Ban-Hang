...
function openTab(nameTab) {
    ...
    switch(nameTab) {
        ...
        case 'Hỗ trợ': document.getElementsByClassName('hotro')[0].style.display = 'block'; break;
        case 'Rút tiền': document.getElementsByClassName('ruttien')[0].style.display = 'block'; break;
...
function layThongTinSanPhamTuTable(id) {
    ...
    // Thông số kỹ thuật (điện máy) - đọc theo input có thuộc tính data-detail
    var detail = {};
    try {
        var inputsDetail = khung.querySelectorAll('input[data-detail]');
        if (inputsDetail && inputsDetail.length) {
            for (var inpDetail of inputsDetail) {
                var k = inpDetail.getAttribute('data-detail');
                if (k) detail[k] = inpDetail.value;
            }
        } else {
            // fallback nếu form cũ (sách)
            ...
            detail = { tacgia: tacgia, xuatsu: xuatsu };
        }
    } catch (e) {
        detail = {};
    }
...
for (var i = 0; i < list_products.length; i++) {
    if (list_products[i].masp == masp) {
        // giữ createdAt cũ nếu có
        sp.createdAt = list_products[i].createdAt || sp.createdAt || Date.now();
        list_products[i] = sp;
    }
}
...
function addKhungSuaSanPham(masp) {
    ...
    var company = Array.from(new Set(list_products.map(p => p.company))).sort();
    ...
    s += `
        ...
        <tr>
            <td>Loại sản phẩm:</td>
            <td><input type="text" data-detail="loai" value="`+(sp.detail && (sp.detail.loai||''))+`"></td>
        </tr>
        <tr>
            <td>Model:</td>
            <td><input type="text" data-detail="model" value="`+(sp.detail && (sp.detail.model||''))+`"></td>
        </tr>
        <tr>
            <td>Công suất / Dung tích:</td>
            <td><input type="text" data-detail="cong_suat" value="`+(sp.detail && (sp.detail.cong_suat||sp.detail.dungtich||sp.detail.dung_tich||sp.detail.khoi_luong_giat||''))+`"></td>
        </tr>
        <tr>
            <td>Công nghệ:</td>
            <td><input type="text" data-detail="cong_nghe" value="`+(sp.detail && (sp.detail.cong_nghe||sp.detail.congnghe||''))+`"></td>
        </tr>
        <tr>
            <td>Bảo hành:</td>
            <td><input type="text" data-detail="bao_hanh" value="`+(sp.detail && (sp.detail.bao_hanh||sp.detail.baohanh||'24 tháng'))+`"></td>
        </tr>

window.onload = function () {
    ...
    // 1) Render khung gợi ý/mới thêm (luôn hiện, không phụ thuộc filter)
    var divGoiY = document.getElementById('goiYChoBan');
    if (divGoiY) {
        divGoiY.innerHTML = '';
        renderGoiYChoBan(soLuong, divGoiY);
    }

    // 2) Render các khung mặc định như cũ
    var yellow_red = ['#ff9c00', '#ec1f1f'];
    var blue = ['#42bcf4', '#004c70'];
    var green = ['#5de272', '#007012'];

    // div khung mặc định: cái contain-khungSanPham thứ 2
    var div = document.getElementsByClassName('contain-khungSanPham')[1];
...
// Thêm sản phẩm vào khung theo "list có sẵn" (không dựa filter URL)
function addKhungSanPhamTuList(list_sanpham, tenKhung, color, len, ele) {
    var gradient = `background-image: linear-gradient(120deg, ${color[0]} 0%, ${color[1]} 50%, ${color[0]} 100%);`;
    var borderColor = `border-color: ${color[0]};`;
    ...
    var l = list_sanpham.slice(0, len);
    for (var i = 0; i < l.length; i++) {
        s += addProduct(l[i], null, true);
    }
...
function renderGoiYChoBan(soLuong, ele) {
    // copy list để sort mà không ảnh hưởng list gốc
    var ds = copyObject(list_products);

    // Ưu tiên theo createdAt (sản phẩm thêm từ admin sẽ có)
    ds.sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));

    // Nếu tất cả đều chưa có createdAt (dữ liệu cũ), lấy “cuối mảng” là mới nhất
    if (!ds.length || (ds[0].createdAt || 0) === 0) {
        ds = copyObject(list_products).reverse();
    }

    addKhungSanPhamTuList(ds, 'GỢI Ý CHO BẠN (MỚI THÊM)', ['#7b2ff7', '#f107a3'], soLuong, ele);
}
